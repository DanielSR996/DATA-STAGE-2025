    ...
    function VistaGeneral() {
        ...
        const exportarAExcel = async () => {
            ...
            loadingDiv.innerHTML = `
                ...
                <p>Este proceso puede tardar varios minutos dependiendo de la cantidad de datos.</p>
            `;
            document.body.appendChild(loadingDiv);

            try {
                // Función para filtrar datos por fecha
                const filtrarPorFecha = (datos, campoFecha) => {
                ...
                const dataFiltrada = {
                    datosGenerales: filtrarPorFecha(datosGenerales, 'Fecha_Pago_Real'),
                    transporteMercancias: filtrarPorFecha(transporteMercancias, 'Fecha_Pago_Real'),
                    guias: filtrarPorFecha(guias, 'Fecha_Pago_Real'),
                    contenedores: filtrarPorFecha(contenedores, 'Fecha_Pago_Real'),
                    facturas: filtrarPorFecha(facturas, 'Fecha_Pago_Real'),
                    fechasPedimento: filtrarPorFecha(fechasPedimento, 'Fecha_Validacion_Pago_Real'),
                    casosPedimento: filtrarPorFecha(casosPedimento, 'Fecha_Validacion_Pago_Real'),
                    cuentasAduanerasGarantiaPedimento: filtrarPorFecha(cuentasAduanerasGarantiaPedimento, 'Fecha_Pago_Real'),
                    tasasPedimento: filtrarPorFecha(tasasPedimento, 'Fecha_Pago_Real'),
                    contribucionesPedimento: filtrarPorFecha(contribucionesPedimento, 'Fecha_Pago_Real'),
                    observacionesPedimento: filtrarPorFecha(observacionesPedimento, 'Fecha_Validacion_Pago_Real'),
                    descargosMercancias: filtrarPorFecha(descargosMercancias, 'Fecha_Pago_Real'),
                    destinatariosMercancia: filtrarPorFecha(destinatariosMercancia, 'Fecha_Pago_Real'),
                    partidas: filtrarPorFecha(partidas, 'Fecha_Pago_Real'),
                    mercancias: filtrarPorFecha(mercancias, 'Fecha_Pago_Real'),
                    permisosPartida: filtrarPorFecha(permisosPartida, 'Fecha_Pago_Real'),
                    casosPartida: filtrarPorFecha(casosPartida, 'Fecha_Pago_Real'),
                    cuentasAduanerasGarantiaPartida: filtrarPorFecha(cuentasAduanerasGarantiaPartida, 'Fecha_Pago_Real'),
                    tasasContribucionesPartida: filtrarPorFecha(tasasContribucionesPartida, 'Fecha_Pago_Real'),
                    contribucionesPartida: filtrarPorFecha(contribucionesPartida, 'Fecha_Pago_Real'),
                    observacionesPartida: filtrarPorFecha(observacionesPartida, 'Fecha_Pago_Real'),
                    rectificaciones: filtrarPorFecha(rectificaciones, 'Fecha_Pago_Real'),
                    diferenciasContribucionesPedimento: filtrarPorFecha(diferenciasContribucionesPedimento, 'Fecha_Pago_Real'),
                    incidenciasReconocimientoAduanero: filtrarPorFecha(incidenciasReconocimientoAduanero, 'Fecha_Seleccion_Automatizada'),
                    seleccionAutomatizada: filtrarPorFecha(seleccionAutomatizada, 'Fecha_Seleccion_Automatizada'),
                    resumen: filtrarPorFecha(resumen, 'Fecha_Ejecucion')
                };

                // Verificar si hay datos para exportar
                const tieneDatos = Object.values(dataFiltrada).some(datos => Array.isArray(datos) && datos.length > 0);
                if (!tieneDatos) {
                ...
                const MAX_REGISTROS_POR_PARTE = 500000;
                const partes = [];
                let parteActual = {};
                let registrosEnParteActual = 0;

                for (const [tabla, datos] of Object.entries(dataFiltrada)) {
                    if (!Array.isArray(datos)) continue;

                    const registrosTabla = datos.length;
                    if (registrosEnParteActual + registrosTabla > MAX_REGISTROS_POR_PARTE && registrosEnParteActual > 0) {
                        partes.push(parteActual);
                        parteActual = {};
                        registrosEnParteActual = 0;
                    }

                    parteActual[tabla] = datos;
                    registrosEnParteActual += registrosTabla;
                }

                if (registrosEnParteActual > 0) {
                    partes.push(parteActual);
                }
                ...
                loadingDiv.innerHTML = `
                    <h3>Exportando datos en ${totalPartes} ${totalPartes === 1 ? 'archivo' : 'archivos'}...</h3>
                    <p>Total de registros: ${Object.values(dataFiltrada).reduce((sum, arr) => sum + (Array.isArray(arr) ? arr.length : 0), 0).toLocaleString()}</p>
                    <p>Progreso: 0/${totalPartes}</p>
                `;
                ...
                for (let i = 0; i < partes.length; i++) {
                    loadingDiv.innerHTML = `
                        <h3>Exportando datos en ${totalPartes} ${totalPartes === 1 ? 'archivo' : 'archivos'}...</h3>
                        <p>Procesando parte ${i + 1} de ${totalPartes}</p>
                    ...
                    const response = await fetch('http://localhost:3000/api/exportar-excel', {
                        ...
                        body: JSON.stringify({
                            ...
                            fechaFin,
                            data: partes[i],
                            parte: totalPartes > 1 ? i + 1 : null,
                            totalPartes: totalPartes > 1 ? totalPartes : null
                        })
                    });

                    if (!response.ok) {
                        const errorText = await response.text();
                        throw new Error(errorText || 'Error al exportar el archivo');
                    }
                    ...
                    a.href = url;
                    a.download = totalPartes > 1 ? 
                        `reporte_${fechaInicio}_${fechaFin}_parte${i + 1}de${totalPartes}.xlsx` :
                ...
                alert(`Exportación completada exitosamente.\nSe generaron ${totalPartes} ${totalPartes === 1 ? 'archivo' : 'archivos'}.`);
            } catch (error) {
