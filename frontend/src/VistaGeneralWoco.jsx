...
function VistaGeneral() {
    ...
    // Componente de diálogo para mostrar detalles
    ...
    // Función para cargar datos de WOCO
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(`http://localhost:3000/api/woco/vista-general?page=${page}`);
            
            if (!response.ok) {
                throw new Error('Error al obtener datos de WOCO');
            }

            const data = await response.json();
            
            // Verificar que los datos sean arrays antes de actualizar los estados
            const updateState = (setter, newData) => {
                if (Array.isArray(newData)) {
                    setter(prevData => [...prevData, ...newData]);
                }
            };

            // Actualizar los estados con los datos recibidos
            updateState(setDatosGenerales, data.datosGenerales || []);
            updateState(setTransporteMercancias, data.transporteMercancias || []);
            updateState(setGuias, data.guias || []);
            updateState(setContenedores, data.contenedores || []);
            updateState(setFacturas, data.facturas || []);
            updateState(setFechasPedimento, data.fechasPedimento || []);
            updateState(setCasosPedimento, data.casosPedimento || []);
            updateState(setCuentasAduanerasGarantiaPedimento, data.cuentasAduanerasGarantiaPedimento || []);
            updateState(setTasasPedimento, data.tasasPedimento || []);
            updateState(setContribucionesPedimento, data.contribucionesPedimento || []);
            updateState(setObservacionesPedimento, data.observacionesPedimento || []);
            updateState(setDescargosMercancias, data.descargosMercancias || []);
            updateState(setDestinatariosMercancia, data.destinatariosMercancia || []);
            updateState(setPartidas, data.partidas || []);
            updateState(setMercancias, data.mercancias || []);
            updateState(setPermisosPartida, data.permisosPartida || []);
            updateState(setCasosPartida, data.casosPartida || []);
            updateState(setCuentasAduanerasGarantiaPartida, data.cuentasAduanerasGarantiaPartida || []);
            updateState(setTasasContribucionesPartida, data.tasasContribucionesPartida || []);
            updateState(setContribucionesPartida, data.contribucionesPartida || []);
            updateState(setObservacionesPartida, data.observacionesPartida || []);
            updateState(setRectificaciones, data.rectificaciones || []);
            updateState(setDiferenciasContribucionesPedimento, data.diferenciasContribucionesPedimento || []);
            updateState(setIncidenciasReconocimientoAduanero, data.incidenciasReconocimientoAduanero || []);
            updateState(setSeleccionAutomatizada, data.seleccionAutomatizada || []);
            updateState(setResumen, data.resumen || []);
            updateState(setUniversoImmex, data.universoImmex || []);

            setTotalRecords(data.pagination?.total || 0);
            setHasMore(page * 50 < (data.pagination?.total || 0));
    ...
    const exportarAExcel = async () => {
        ...
        try {
            const response = await fetch('http://localhost:3000/api/woco/exportar-excel', {
                method: 'POST',
                headers: {
            ...
            a.href = url;
            a.download = `reporte_woco_${fechaInicio}_${fechaFin}.xlsx`;
    ...
    // Componente de diálogo para detalles
    const DetallesDialog = ({ open, onClose, row }) => {
        if (!row) return null;
                ...
                <Container maxWidth={false}>
                    <Paper elevation={3} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
                        <Typography variant="h5" component="h1" gutterBottom sx={{ 
                            ...
                            Vista General de Pedimentos - WOCO
                        </Typography>
